...
public class MyJobCollectionAdapter extends BaseRecyclerAdapter<MyJobCollectionAdapter.MyViewHolder> implements View.OnClickListener {
    ...
    private List<JobCollectionBean.DataBean> list;
    private String api_token;
    private Context context;

    public MyJobCollectionAdapter(List<JobCollectionBean.DataBean> list, Context context) {
        this.list = list;
    ...
    // 重新加载所有数据
    public void setData(List<JobCollectionBean.DataBean> totaList, boolean isClear) {
        if (isClear) {
            list.clear();
        }
    ...
    private void cancelCollectionJob(final int position) {
        ...
        formEnding.add("com_id", list.get(position).getId()+"");
        formEnding.add("job_id", list.get(position).getJob_id());
        OkhttpUtils.getInstance().sendPostHttp(ContentUrl.BASE_URL + ContentUrl.FAVORITES_DELETE, api_token, formEnding, new Callback() {
            @Override
