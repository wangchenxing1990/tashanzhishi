...
public class MyJobSearchAdapter extends BaseRecyclerAdapter<MyJobSearchAdapter.MyViewHolder>implements View.OnClickListener {
    ...
    private List<SearchResultDataBean.DataBean> list;
    private Context context;
    public MyJobSearchAdapter(List<SearchResultDataBean.DataBean> list, Context context) {
        this.list = list;
        this.context = context;
    }
    // 重新加载所有数据
    public void setData(List<SearchResultDataBean.DataBean> totaList, boolean isClear) {
        if (isClear) {
            list.clear();
        }
...
//                .into(holder.image_left);
    ...
    public void onClick(View v) {
        if (mOnItemClickListener != null) {
            //注意这里使用getTag方法获取数据
            mOnItemClickListener.onItemClick(v,(SearchResultDataBean.DataBean)v.getTag());
        }
    }

    public static interface OnRecyclerViewItemClickListener {
        void onItemClick(View view, SearchResultDataBean.DataBean data);
    }
