...
public class MyJobBrowseAdapter extends BaseRecyclerAdapter<MyJobBrowseAdapter.MyViewHolder> implements View.OnClickListener {
    ...
    private List<JobDestoryDataBean.DataBean> list;

     private Context context;
    public MyJobBrowseAdapter(List<JobDestoryDataBean.DataBean> list, Context context) {
        this.list = list;
        this.context = context;
    }

    // 重新加载所有数据
    public void setData(List<JobDestoryDataBean.DataBean> totaList, boolean fresh) {
        if (fresh) {
            list.clear();
        }
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position, boolean isItem) {
        ...
        holder.text_salary.setText(list.get(position).getSalary()+"元/月");
            ...
            if (list.get(position).getIs_urgent()==1) {
            ...
            } else if (list.get(position).getIs_urgent()==2) {
            ...
            } else if (list.get(position).getIs_urgent()==3){
                holder.text_part_states.setText("兼职 ");
                holder.text_part_states.setTextColor(0xff42bfb6);
                holder.text_part_states.setVisibility(View.VISIBLE);
                holder.text_part_states.setBackgroundResource(R.drawable.shape_ovil_green);
            } else if (list.get(position).getIs_urgent()==4) {
            ...
            } else if (list.get(position).getIs_urgent()==0) {
                holder.text_part_states.setVisibility(View.INVISIBLE);
            }
    ...
    public void onClick(View v) {
        if (mOnItemClickListener != null) {
            //注意这里使用getTag方法获取数据
            mOnItemClickListener.onItemClick(v, (JobDestoryDataBean.DataBean) v.getTag());
        }
    }

    public static interface OnRecyclerViewItemClickListener {
        void onItemClick(View view, JobDestoryDataBean.DataBean data);
    }
