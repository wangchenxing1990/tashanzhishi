...
import com.wangyukui.ywkj.adapter.MyReceivePartAdapter;
...
public class ReceiveActivity extends BaseActiviyt {
    ...
    private void changePartApplyStatue(PartApplyBean.DataBean dataBean) {
        ...
        form.add("status","1");
        OkhttpUtils.getInstance().sendPostHttp(ContentUrl.BASE_URL + ContentUrl.PART_SIGN_PROCESS_UPDATE, api_token, form, new Callback() {
            @Override
    ...
    private void refreshCurrentPageOne() {
        FormEncodingBuilder form = new FormEncodingBuilder();
        OkhttpUtils.getInstance().sendPostHttp(ContentUrl.BASE_URL + ContentUrl.PART_SIGN_PROCESS_RECEIVE, api_token, form, new Callback() {
            @Override
            public void onFailure(Request request, IOException e) {
